...
//        Long id = namedParameterJdbcTemplate.query(strQuery, map, (PreparedStatementCallback) ps -> ps.executeUpdate());
    ...
    @Override
    public void registrationMessage(String text, Timestamp timestamp, String sender) {
        String sql = "insert into day09.messages (time, sender, text) values (:timestamp, :sender, :text);";
        namedParameterJdbcTemplate.update(sql, new MapSqlParameterSource().addValue("timestamp", timestamp).addValue("sender", sender).addValue("text", text));
    }
